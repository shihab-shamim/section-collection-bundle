...
const Settings = ({ attributes, setAttributes, device, clientId }) => {
	...
	// console.log(attributes.selectedTheme);
	...
	return <>
		...
		<BlockControls>
			...
			<AlignmentToolbar value={alignment} onChange={val => setAttributes({ alignment: val })} describedBy={__('Block Name Alignment')} alignmentControls={[
				{ title: __('left', 'b-blocks'), align: 'left', icon: 'align-left' },
				{ title: __('center', 'b-blocks'), align: 'center', icon: 'align-center' },
				{ title: __('right', 'b-blocks'), align: 'right', icon: 'align-right' }
			]} />
 
			<BplBlockPreview blocks={themes} clientId={clientId} value={attributes.selectedTheme} onChange={(value) =>setAttributes({selectedTheme: updateData(attributes, value, "selectedTheme"),})
          }/>
